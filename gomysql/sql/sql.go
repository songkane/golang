...
func (dc *driverConn) lifetimeEnded(timeout time.Duration) bool {
	if timeout <= 0 {
		return false
	}
...
func (dc *driverConn) idleExpired(timeout time.Duration) bool {
	if timeout <= 0 {
		return false
	}
	return !dc.idledAt.IsZero() && dc.idledAt.Add(timeout).Before(nowFunc())
}
...
// SetConnWaitTimeout sets the maximum amount of time a connRequest may wait to be satisfied.
//
// Expired connRequest may be closed immediately.
//
// If d <= 0, ConnWaitTimeout will not updated.
//
// Default ConnWaitTimeout is 3 seconds
func (db *DB) SetConnWaitTimeout(d time.Duration) {
	if d <= 0 {
		return
	}
	db.mu.Lock()
	db.connWaitTimeout = d
	db.mu.Unlock()
}

// SetConnIdleTimeout sets the maximum time that an idle connection can remain idle in the pool.
//
// Expired idle connections may be closed immediately.
//
// If d <= 0, idle connections will not be removed.
func (db *DB) SetConnIdleTimeout(d time.Duration) {
	if d < 0 {
		d = 0
	}
	db.mu.Lock()
	// wake cleaner up when idle timeout is shortened.
	if d >= 0 && d < db.connIdleTimeout && db.cleanerCh != nil {
		select {
		case db.cleanerCh <- struct{}{}:
		default:
		}
	}
	db.connIdleTimeout = d
	db.mu.Unlock()
}

// startCleanerLocked starts connectionCleaner if needed.
func (db *DB) startCleanerLocked() {
	if d := db.decideCleanIntervalLocked(); d >= 0 && db.numOpen > 0 && db.cleanerCh == nil {
		db.cleanerCh = make(chan struct{}, 1)
		go db.connectionCleaner(d)
	}
}

func (db *DB) decideCleanIntervalLocked() time.Duration {
	cleanInterval := time.Duration(0)
	if db.maxLifetime <= 0 {
		cleanInterval = db.connIdleTimeout
	} else if db.connIdleTimeout <= 0 {
		cleanInterval = db.maxLifetime
	} else if db.maxLifetime > db.connIdleTimeout {
		cleanInterval = db.connIdleTimeout
	}
...
func (db *DB) connectionCleaner(d time.Duration) {
	...
	for {
		...
		db.mu.Lock()
		d = db.decideCleanIntervalLocked()
		...
		lifetimeEndedSince := nowFunc().Add(-db.maxLifetime)
		idleExpiredSince := nowFunc().Add(-db.connIdleTimeout)
		var closing []*driverConn
		for i := 0; i < len(db.freeConn); i++ {
			c := db.freeConn[i]
			if (db.maxLifetime > 0 && c.createdAt.Before(lifetimeEndedSince)) ||
				(db.connIdleTimeout > 0 && c.idledAt.Before(idleExpiredSince)) {
...
var errConnWaitTimeout = errors.New("sql: database connection wait timeout")

var defaultConnWaitTimeout = time.Second * 3
var dummyIdledAt = time.Time{}
...
func (db *DB) conn(ctx context.Context, strategy connReuseStrategy) (*driverConn, error) {
	...
	idleTimeout := db.connIdleTimeout
	...
	if strategy == cachedOrNewConn && numFree > 0 {
		...
		conn.inUse = true
		db.mu.Unlock()
		if conn.idleExpired(idleTimeout) || conn.lifetimeEnded(lifetime) {
			conn.Close()
			return nil, driver.ErrBadConn
		}
		conn.idledAt = dummyIdledAt
		return conn, nil
	}
	...
	if db.maxOpen > 0 && db.numOpen >= db.maxOpen {
		...
		if db.connWaitTimeout == 0 {
			db.connWaitTimeout = defaultConnWaitTimeout
		}
		...
		case <-ctx.Done():
			...
			close(req) // close req channel
			return nil, ctx.Err()
		case <-time.After(db.connWaitTimeout):
			db.mu.Lock()
			delete(db.connRequests, reqKey)
			db.mu.Unlock()
			select {
		...
		case ret, ok := <-req:
			...
			if ret.err == nil && ret.conn.lifetimeEnded(lifetime) {
				ret.conn.Close()
				return nil, driver.ErrBadConn
			}
