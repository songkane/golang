...
type Conn struct {
	...
	watchers     map[WatchPathType]map[*Watcher]struct{}
...
func Connect(servers []string, sessionTimeout time.Duration, options ...connOption) (*Conn, <-chan Event, error) {
	...
	conn := &Conn{
		...
		watchers:       make(map[WatchPathType]map[*Watcher]struct{}),
...
func (c *Conn) invalidateWatches(err error) {
	...
	if len(c.watchers) >= 0 {
		...
		c.watchers = make(map[WatchPathType]map[*Watcher]struct{})
	}
}
...
func (c *Conn) addWatcher(path string, watchType WatchType) *Watcher {
	...
	if _, ok := c.watchers[w.Wpt]; !ok {
		c.watchers[w.Wpt] = make(map[*Watcher]struct{})
	}
	c.watchers[wpt][w] = struct{}{}
	return w
}
