...
func (bot *BotAPI) MakeRequest(endpoint string, params url.Values) (APIResponse, error) {
	...
	golog.Info(fmt.Sprintf("bot MakeRequest, endpoint:%s, resp:%s", string(endpoint), string(bytes)))
...
func (bot *BotAPI) decodeAPIResponse(responseBody io.Reader, resp *APIResponse) (data []byte, err error) {
...
func (bot *BotAPI) debugLog(context string, v url.Values, message interface{}) {
	if bot.Debug {
		golog.Debug(fmt.Sprintf("%s req : %+v\n", context, v))
		golog.Debug(fmt.Sprintf("%s resp: %+v\n", context, message))
	}
}
...
func (bot *BotAPI) GetUpdatesChan(config UpdateConfig) (UpdatesChannel, error) {
	...
	go func() {
		for {
			...
			if err != nil {
				golog.Error(fmt.Sprintf("bot GetUpdatesChan error: %v", err))
				golog.Error("bot GetUpdatesChan Failed to get updates, retrying in 3 seconds...")
